package com.shd0w.study.StarWarsApi.service.Planet.impl;

import com.shd0w.study.StarWarsApi.model.PlanetModel;
import com.shd0w.study.StarWarsApi.service.Planet.interfaces.PlanetInterface;
...
@Service
@Slf4j
public class PlanetService implements PlanetInterface {
    private static final URI endpointUri = URI.create("https://swapi.dev/api/planets");

    RestTemplate restTemplate = new RestTemplate();

    @Override
    public PlanetModel getAllPlanets(@Nullable Integer page, @Nullable String searchQuery) {
        URI uri = getUri(page, searchQuery, endpointUri, null);

        try {
            PlanetModel response = restTemplate.getForObject(uri.toString(), PlanetModel.class);
    ...
    @Override
    public PlanetModel getPlanetById(@Nullable Integer id) {
